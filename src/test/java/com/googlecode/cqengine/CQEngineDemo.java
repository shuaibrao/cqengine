...
public class CQEngineDemo {
    ...
    public static void main(String[] args) {
        ...
        query = all(Car.class);
        printResults(cars.retrieve(query, queryOptions(orderBy(ascending(Car.CAR_ID)))));

        System.out.println("\nFord cars:");
        query = equal(Car.MANUFACTURER, "Ford");
        printResults(cars.retrieve(query));
        ...
        query = not(equal(Car.DOORS, 3));
        printResults(cars.retrieve(query, queryOptions(orderBy(ascending(Car.DOORS)))));

        System.out.println("\nNOT 3-door cars, sorted by doors ascending then price descending:");
        query = not(equal(Car.DOORS, 3));
        printResults(cars.retrieve(
                query, queryOptions(orderBy(ascending(Car.DOORS), descending(Car.PRICE)))));
    ...
    static void printResults(ResultSet<Car> results) {
        try (ResultSet<Car> closeableResults = results) {
            closeableResults.forEach(System.out::println);
        }
    }
}
